...
export class Sqlite {
  ...
  public static async instantiate(
    vfs: Vfs,
    { isDebug = false }: SqliteOptions | undefined = { isDebug: false }
  ): Promise<Sqlite> {
    let exports: Exports;
      ...
      fetch(new URL("./wasm_sqlite.wasm", import.meta.url)),
      {
        wasi_snapshot_preview1: {
          ...
          fd_write(
            ...
            for (let i = 0; i < iovsLength; i++) {
              ...
              switch (fd) {
                case 1: // stdout
                  if (isDebug) stdout.write(s);
                  break;
                case 2: // stderr
                  if (isDebug) stderr.write(s);
                  break;
                default:
                  return ERRNO_BADF;
              }
        ...
        env: {
          ...
          async get_page(ix: number, ptr: number) {
            ...
            if (isDebug) console.log("got page:", ix, page);
            if (isDebug) console.log("write at", ptr, page.length);
          ...
          async conn_sleep(ms: number) {
            if (isDebug) console.log("sleep", ms);
